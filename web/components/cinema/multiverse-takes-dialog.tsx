...
import { Film, Clapperboard, Check, Sparkles, Layers, Sliders } from "lucide-react";
...
export const ALTERNATE_TAKES: MultiverseTake[] = [
  {
...
export function MultiverseTakesDialog({
  ...
  const activeTake = ALTERNATE_TAKES.find((t) => t.id === selectedTakeId) || ALTERNATE_TAKES[0];
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-6 border-border bg-card">
        <DialogHeader className="border-b border-border pb-3">
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            Explore 3 diverging director cuts for this scene. Each take alters tone, subtext ratio, camera blocking, and dialogue rhythm.
          </DialogDescription>
        </DialogHeader>

        {/* 3 Takes Selection Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5 pt-2">
          {ALTERNATE_TAKES.map((take) => {
