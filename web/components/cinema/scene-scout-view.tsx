...
export function SceneScoutView({
  ...
  const handleGenerateImage = async () => {
    ...
    const promptToUse =
      customPrompt.trim() ||
      `Cinematic establishing frame of ${activeScene.location || activeScene.title}, ${activeScene.slugline}. Master Hollywood lighting, photoreal.`;
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (result.image_url) {
          ...
          setHeroPrompt(promptToUse);
          setHeroTitle(activeScene.title);

          // Append to scene's sceneImages list
          const newImgEntry = {
            id: `img-${Date.now()}`,
            url: result.image_url,
            prompt: promptToUse,
            createdAt: Date.now(),
            title: `${activeScene.title} — Look ${Date.now().toString().slice(-4)}`,
          ...
          const updatedScene: FilmScene = {
            ...activeScene,
            sceneImages: [newImgEntry, ...existingImgs],
            preview_image_url: activeScene.preview_image_url || result.image_url,
          };

          onUpdateScene?.(updatedScene);
          notifyIfFallback(result, "Scene Concept Look");
          toast.add({
            title: "Scene Image Rendered",
            description: `Generated cinematic concept frame for Scene ${activeScene.sceneNumber}.`,
            type: "success",
          });
        } else {
          toast.add({ title: "Generation failed", description: "No image URL returned.", type: "error" });
        }
      } else {
        toast.add({ title: "Generation failed", description: `API returned status ${res.status}`, type: "error" });
      }
    } catch (err) {
      console.error("Scene image generation error:", err);
      toast.add({
        title: "Image generation error",
        description: err instanceof Error ? err.message : "Failed to reach generation service.",
        type: "error",
      });
    } finally {
                ...
                <div className="absolute inset-0 bg-black/85 backdrop-blur-xs flex flex-col items-center justify-center gap-3 z-30">
                  ...
                  <div className="text-center">
                    ...
                    <span className="text-xs font-mono text-muted-foreground">
                      Imagen 3 High-Fidelity Photoreal Synthesis
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="h-36 shrink-0 border-t border-border bg-card/60 px-4 py-2 flex flex-col justify-between overflow-hidden">
            ...
            <div className="flex-1 min-h-0 flex items-center gap-2.5 overflow-x-auto pb-0.5 scrollbar-thin">
              ...
              <button
                ...
                title="Synthesize another cinematic look with Imagen 3"
              >
            ...
            >
              {isGenerating ? (
                <>
                  ...
                  <span>Synthesizing Concept Image (Imagen 3)...</span>
                </>
              ) : (
                <>
                  <Sparkles className="h-4 w-4" />
                  <span>Generate Scene Concept Image</span>
                </>
              )}
            </Button>
          </div>
